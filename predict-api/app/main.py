...
async def predict(text_content: str, model_type: str = "naive_bayes") -> Union[str, float]:
    ...
    if model_type is None:
        return {"message": "É necessário enviar um modelo para realizar a predição. Utilize naive-bayes ou rede-neural"}
    if model_type in ['naive_bayes', 'rede_neural']:
        ...
        return {"message": "É necessário enviar um modelo para realizar a predição. Utilize naive-bayes ou rede-neural"}
@app.get("/metrics", tags=["predict"], summary="Métricas", description="""Endpoint para realizar métricas da API.""", response_description="Métricas", response_model= str)
async def metrics() -> str:
    return "Métricas da API de predição de sentimento de textos"

@app.get("/healthcheck", tags=["predict"], summary="Healthcheck", description="""Endpoint para realizar healthcheck da API.""",
        response_description="Healthcheck", response_model= str)
